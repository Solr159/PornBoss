...
// Jav stores metadata fetched from JavBus for a given code (may map to multiple videos).
type Jav struct {
...
type JavTag struct {
	...
	IsUser    bool      `json:"is_user" gorm:"not null;default:false;uniqueIndex:idx_jav_tag_name_source"`
	CreatedAt time.Time `json:"created_at"`
	UpdatedAt time.Time `json:"updated_at"`
}
